...
namespace LandTitleRegistration.Services
{
    public class TitleService
    {
        ...
        {
            // Retrieve API key from Azure Key Vault instead of hard-coded value
            if (_secretClient != null)
            {
                try
                {
                    var secret = await _secretClient.GetSecretAsync("GovApiKey");
        ...
        {
            // Use UTC time instead of DateTime.Now for cloud consistency
            var registeredDate = DateTimeOffset.UtcNow;
            var titleRef = "LT-" + registeredDate.ToUnixTimeMilliseconds().ToString();

            // Use Entity Framework Core with parameterized queries (no SQL injection risk)
        ...
        {
            // Use Entity Framework Core with LINQ (parameterized, no SQL injection)
        ...
        {
            // Retrieve API key from Key Vault and use configured service URL
            var apiKey = await GetApiKeyAsync();
            var govReportingUrl = _configuration["ServiceUrls:GovReportingApi"];
            
            var url = $"{govReportingUrl}?month={month}&year={year}&apiKey={apiKey}";
